...
- 각 슬라이드는 시각 요소의 역할을 명확히 정한다: `none`, `imagegen-infographic`, `chart/table`, `screenshot`, `photo/reference`, `native-label-region`.
- 생성형 hero, concept, process, service-flow, risk visual은 반드시 `image-prompt`로 컴파일하고 upstream validator를 통과한 뒤 Codex `$imagegen`(`gpt-image-2`)으로 생성한다.
- 마케팅/제품 소개 성격의 hero visual, product mockup, OG/social preview 이미지는 `imagegen-marketing-assets.md`의 단일 provider 계약을 함께 적용한다.
- `native-diagram`, HTML/SVG, PPT 도형만으로 만든 그림은 인포그래픽으로 인정하지 않는다. 도형은 한국어 라벨, 콜아웃, 연결선, 데이터 차트, 표, 보조 overlay에만 사용한다.
- 흐름, 비교, 단계, 구조, 의사결정 기준을 설명하는 슬라이드는 먼저 visual contract를 작성하고 `image-prompt`의 C6/C12 라우팅에 맡긴다.
- 정확한 순서, 수치, 라벨, 표 구조가 핵심인 단계 카드나 데이터 차트는 `imagegen-infographic`이 아니라 `native-label-region` 또는 `chart/table`로 분류한다. 생성 raster 위에 native text를 덧씌우지 않고, 편집 가능한 정보는 별도 layout 영역에 둔다.
...
- Codex `$imagegen`을 실행하지 못한 경우 다른 모델, placeholder, native diagram, 임시 SVG를 납품용 생성 이미지처럼 대체하지 않는다. `blocked_imagegen_not_run`으로 기록하고 사용자에게 상태를 알린다.
...
Prompt를 이 문서에서 조립하지 않는다. asset brief와 deck evidence를
`image-prompt`에 전달하고, Gongnyang 원본이 만든 `full_prompt`를 validator로
확인한 뒤 Codex host tool의 `prompt` 필드로 수정 없이 매핑한다. 텍스트가 필요한 생성 이미지는
Gongnyang의 text rendering 규칙으로 이미지 안에서 만들고, 편집 가능한 PPT
본문은 생성 raster와 겹치지 않는 native region으로 분리한다.
