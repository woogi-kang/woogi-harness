...
description: "Future Slide 덱에 들어갈 hero image, diagram base, icon 등 텍스트 없는 보조 이미지를 생성하고 asset manifest를 남기는 스킬."
argument-hint: "[asset-brief]"
metadata:
  category: "📝 콘텐츠"
  version: "0.1.0"
  tags: "presentation, image-generation, codex-native-image, cli, assets, future-slide"
  author: "woogi"
---
...
- CLI/native image generation을 실행하지 않았는데 "생성 이미지"처럼 보이는 local placeholder를 납품용으로 만드는 것
- 텍스트를 제거한 결과, 무엇을 설명하는지 알 수 없는 이미지

대체:
- PPT/HTML 덱에서 인포그래픽으로 분류되는 비주얼은 imagegen으로 생성한 bitmap asset을 사용합니다.
- 한국어 라벨, 숫자, 표, 축, 설명문은 이미지 안에 넣지 않고 HTML/PPT 텍스트 레이어, 차트, 표, 콜아웃으로 분리합니다.
- Codex imagegen/native generation/CLI를 실제 실행할 수 없으면 `status: "blocked_imagegen_not_run"`로 기록하고, 납품용 이미지를 만들지 않습니다.
...
분류 원칙:
- 인포그래픽으로 분류한 시각 요소는 반드시 imagegen으로 만든 bitmap asset을 사용합니다.
- 정확한 순서, 수치, 한국어 라벨이 핵심인 단계 카드와 차트는 인포그래픽 대체물이 아니라 `native-label-overlay` 또는 `chart/table`로 분류합니다.
- 이 경우 imagegen은 배경/상황 visual로만 사용하고, 순서와 라벨은 HTML/PPT 텍스트 레이어로 둡니다.

## CLI / Native Image Generation

이 스킬은 기본적으로 `imagegen` 스킬의 built-in `image_gen`을 사용합니다. CLI/API fallback은 사용자가 명시적으로 요청하거나 승인한 경우에만 사용합니다.

원칙:
- Codex built-in `image_gen` 또는 승인된 CLI workflow를 사용합니다.
...
- manifest에는 built-in 사용 시 `generator: "codex-native-imagegen"`, CLI 사용 시 `generator: "codex-native-image-cli"`를 기록합니다.
- 실행하지 못한 경우에는 `status: "blocked_imagegen_not_run"`로 기록합니다.

## Prompt 규칙

모든 prompt에 기본 제약을 포함합니다.

```text
No text, no letters, no numbers, no UI labels, no watermark.
Leave clean negative space for Korean PPT text or HTML typography.
Match the deck accent color and restrained presentation style.
```

Slot별 권장:
- `s22-hero-21x9`: wide hero, 21:9, strong negative space, single focal metaphor
- `s15-grid-21x9`: repeated analytical scene or abstract system part, consistent crop
- `s16-brief-16x10`: compact icon-like visual, simple background, no detail clutter
- icon set: transparent or flat background, one stroke/fill language, no text
...
[
  {
    ...
    "generator": "codex-native-image-cli",
    "prompt": "No text...",
    "alt": "검색 결과가 답변 엔진으로 재구성되는 추상적 네트워크",
    "crop": "center center",
    "status": "generated_via_cli",
    "visual_contract": {
