...
- 제목과 핵심문장/lede 사이 간격이 슬라이드마다 크게 달라지지 않는지 확인합니다.
- 같은 위계의 좌우 박스, 상단/하단 바, 프로세스 카드가 같은 기준선과 폭을 쓰는지 확인합니다.
- 제목 옆 라인, 번호 밑줄, shadow, 장식용 divider가 의미 없이 남아 있으면 수정 대상으로 표시합니다.
- 한 프로세스 단계만 accent color로 강조되어 있으면 그 의미가 명시되어 있는지 확인합니다.
- 문장형 설명이 중앙 정렬되어 읽기 흐름을 해치지 않는지 확인합니다.
- 주요 본문이 8-9pt대처럼 발표 화면에서 읽기 어려운 크기로 축소되지 않았는지 확인합니다.
- 추상 개념 슬라이드에 실제 질문/입력, 자연어 답변, 구조화 결과, 실패/검증 이유가 있는지 확인합니다.
- `방지 방향`, `서비스 데이터`, `앱이 읽는다`처럼 모호한 표현이 실제 동작으로 바뀌었는지 확인합니다.
