...
- 생성 이미지는 `image-prompt`가 컴파일한 hero visual, diagram base, icon, texture, object illustration로 제한합니다.
- S22는 21:9 hero, S15/S16은 21:9 또는 16:10 slot을 우선 사용합니다.
- 이 스킬에서 이미지 prompt를 작성하지 않습니다. Slot, deck palette, claim, 실제 evidence를 `image-prompt`에 넘깁니다.
- 생성 raster 위에 HTML/PPT text를 덧씌우지 않습니다. 편집 가능한 본문은 별도 layout region에 둡니다.
