...
rollback() {
  ...
  APPLIED=0
  echo "ROLLBACK: restored previous img2threejs vendor" >&2
}
...
TMP="$(mktemp -d "$ROOT/.img2threejs-update.XXXXXX")"
...
git -C "$UPSTREAM" archive --format=tar "$COMMIT" -- "${RUNTIME_PATHS[@]}" |
  tar -xf - -C "$STAGED_VENDOR"
cp "$VENDOR/UPSTREAM.md" "$STAGED_VENDOR/UPSTREAM.md"
git -C "$UPSTREAM" ls-tree -r "$COMMIT" -- "${RUNTIME_PATHS[@]}" \
  >"$STAGED_VENDOR/UPSTREAM.manifest"
...
skill_version="$(
  python3 - "$STAGED_VENDOR/SKILL.md" <<'PY'
import re
import sys
...
grep -q '^                                 Apache License$' "$STAGED_VENDOR/LICENSE"
grep -q '^name: img2threejs$' "$STAGED_VENDOR/SKILL.md"
python3 -m json.tool "$STAGED_VENDOR/UPSTREAM.lock.json" >/dev/null
PYTHONPYCACHEPREFIX="$TMP/pycache" python3 -m compileall -q "$STAGED_VENDOR/forge"
python3 "$STAGED_VENDOR/forge/stage1_intake/probe_image.py" --help >/dev/null
...
bash "$ROOT/scripts/verify-img2threejs.sh"
APPLIED=0
rm -rf "$TMP/backup"
echo "UPDATED: img2threejs -> $COMMIT"
